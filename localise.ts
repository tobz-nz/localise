...
async function resolvedPackageVersion(
  ...
  const pkgJsonPath = `${projectDir}/node_modules/${name}/package.json`;
  try {
  ...
  throw new Error(
    `Could not read resolved version for ${name} (expected ${pkgJsonPath})`,
  );
}

function outputHeader(packageLabel: string): string {
  const tool =
    LOCALISE_VERSION === "dev" ? "localise dev" : `localise v${LOCALISE_VERSION}`;
  return [
    "/**",
    ` * Generated by ${tool}`,
...
try {
  ...
  const name = npmPackageName(pkg);
  const version = await resolvedPackageVersion(tempDir, name);
  await Deno.writeTextFile(
    absoluteOut,
    outputHeader(`${name}@${version}`) + codeText,
  );
  console.log(`Localised ${name}@${version} → ${out}`);
} catch (error) {
